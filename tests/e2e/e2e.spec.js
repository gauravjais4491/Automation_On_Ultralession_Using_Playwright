...
for (const data of e2eData) {
    customTest(`should place an order with product name ' ${data.productName} '`, async ({ homePage, storePage, page, productDetailsPage, viewCartPage, shoppingAddressPage, paymentDetailsPage }) => {
        ...
        await customTest.step('should go to store page', async ({},testInfo) => {
            console.log(testInfo.title);
            await homePage.goToStorePage()
        })

        await customTest.step('should go to product details page', async ({},testInfo) => {
            console.log(testInfo.title);
            await storePage.goToProductDetailsPage(data.productName)
        })

        await customTest.step('should add product to cart', async ({},testInfo) => {
            console.log(testInfo.title);
            await productDetailsPage.addProductToCart(data.numberOfItems)
        })

        await customTest.step('should go to shopping address page', async ({},testInfo) => {
            console.log(testInfo.title);
            await viewCartPage.goToShoppingAddressPage();
        })
        await customTest.step('should add contact details', async ({},testInfo) => {
            console.log(testInfo.title);
        ...
        await customTest.step('should add payment details', async ({},testInfo) => {
            console.log(testInfo.title);
